...
- [ ] **Deploy production**: apply migration 0006 lên D1 production, set secret `OPENAI_API_KEY`/`OPENAI_BASE_URL` cho production, `gsk hosted deploy` (hoặc flow deploy tương ứng), rồi test lại AI chat + Camera CRUD trên URL production thật.
...
- [ ] **Test UI qua browser thật** (click-through) — mới verify qua API call trực tiếp + kiểm tra HTML/JS được nạp đúng, chưa bấm nút trên trình duyệt thật (thiếu `libatk-1.0.so.0` trong sandbox để chạy Playwright/headless Chromium).
...
- [ ] Cân nhắc bổ sung polling 15s (giống dashboard) cho lưới camera/feed cảnh báo để tự cập nhật mà không cần bấm lại vào mục.
...
- **Cập nhật lần cuối**: 2026-07-17 (thêm AI thật + phân hệ An ninh Camera — backend + frontend hoàn tất trên sandbox, CHƯA redeploy production)
